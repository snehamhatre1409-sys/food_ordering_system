...
# 🔥 ONE-TIME DATABASE MIGRATION - Run this once!
@st.cache_data(ttl=300)  # Cache for 5 mins during testing
def migrate_db():
    c = conn.cursor()
...
if st.sidebar.checkbox("🔧 Run DB Migration (one-time)"):
    migrate_db()
...
def init_db():
    ...
    # All your table creation code here...
...
# ✅ FIXED: Get connection globally
conn = init_db()

# Migration function (uses global conn)
def migrate_db():
    c = conn.cursor()  # ✅ Now works!
...
conn = init_db()  # ← Global connection
        ...
        if st.session_state.cart:
            ...
            if st.button("Place Order"):
                ...
                c.execute("INSERT INTO orders (user_id, total, items) VALUES (?, ?, ?)", 
                          (st.session_state.user_id, total, items_json))
                conn.commit()
                st.session_state.cart = []
...
orders_df = pd.read_sql(f"""
    SELECT * FROM orders WHERE user_id={st.session_state.user_id} ORDER BY order_date DESC
""", conn)
...
def admin_dashboard():
    ...
    with tab2:
        ...
        for idx, row in orders_df.iterrows():
            ...
            with col1:
                st.button("Complete", key=f"complete_{row['id']}", on_click=lambda id=row['id']: update_status(id, 'completed'))
            with col2:
                st.button("Cancel", key=f"cancel_{row['id']}", on_click=lambda id=row['id']: update_status(id, 'cancelled'))
    with tab3:
        revenue_df = pd.read_sql("SELECT SUM(total) as revenue, COUNT(*) as orders FROM orders WHERE status='completed'", conn)
        col1, col2 = st.columns(2)
        col1.metric("Total Revenue", f"₹{revenue_df['revenue'].iloc[0] or 0}")
        col2.metric("Total Orders", revenue_df['orders'].iloc[0] or 0)
        fig = px.pie(values=[len(orders_df[orders_df['status']=='pending']), len(orders_df[orders_df['status']=='completed'])], 
                     names=['Pending', 'Completed'], title="Order Status")
        st.plotly_chart(fig)
...
# Sidebar logout
st.sidebar.button("Logout", on_click=lambda: (setattr(st.session_state, 'logged_in', False), setattr(st.session_state, 'user_id', None), st.rerun()))
